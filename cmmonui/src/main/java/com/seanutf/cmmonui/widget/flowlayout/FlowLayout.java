package com.seanutf.cmmonui.widget.flowlayout;
 ...
 * 目前仅仅适用所使用的地方，修改记录参见git commit
 */
public class FlowLayout extends RelativeLayout {

    // 水平间距，单位为dp
    private int horizontalSpacing = dp2px(8);
    // 垂直间距，单位为dp
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (attr == R.styleable.FlowLayoutAttrs_horizontalSpacing) {
                horizontalSpacing = typedArray.getDimensionPixelSize(attr, dp2px(0));

            } else if (attr == R.styleable.FlowLayoutAttrs_verticalSpacing) {
                verticalSpacing = typedArray.getDimensionPixelSize(attr, dp2px(8));

            } else if (attr == R.styleable.FlowLayoutAttrs_itemSize) {
                textSize = typedArray.getDimensionPixelSize(attr, dp2px(15));

            } else if (attr == R.styleable.FlowLayoutAttrs_itemColor) {
                textColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.FlowLayoutAttrs_backgroundResource) {
                backgroundResource = typedArray.getResourceId(attr, 0);

            } else if (attr == R.styleable.FlowLayoutAttrs_textPaddingH) {
                textPaddingH = typedArray.getDimensionPixelSize(attr, dp2px(0));

            } else if (attr == R.styleable.FlowLayoutAttrs_textPaddingV) {
                textPaddingV = typedArray.getDimensionPixelSize(attr, dp2px(0));
            }
        }
        typedArray.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 加上行的竖直间距
        totalHeight += verticalSpacing * (size - 1);
        // 加上上下padding
